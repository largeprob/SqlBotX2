...
import { CodeBrackets, ExpandDown, Me, PauseOne, Send, UploadLogs, Windows, Error as ErrorIcon, DataArrival } from '@icon-park/react';
...
import type { ChatMessage, ContentBlockText } from '@/types/messages';
...
const TextRenderer = ({ block }: { block: any }) => {
    return (
        <div className="text-sm  text-gray-800 leading-relaxed whitespace-pre-wrap pt-2 pb-2">
            {block.text}
...
const MessageItem = ({ message }: { message: ChatMessage }) => {
    ...
    return (
        <div className={`flex gap-2 w-full ${isUser ? 'flex-row-reverse' : 'flex-row'} animate-in fade-in slide-in-from-bottom-2`}>
            ...
            {
                isAssistant &&
                <div className={`relative w-full  px-0   flex flex-col items-start  `}>
                    {/* 时间 */}
                    <div className='text-[10px] p-2 text-gray-400'>
                        9:50:21
                    </div>
                        ...
                        <CardBody>
                            <Accordion selectionMode="multiple" defaultExpandedKeys={["1"]}>
                                <AccordionItem
                                        ...
                                        <>
                                            {
                                                message.status === 'streaming' && (
                                                    <div className="flex items-center gap-1.5 mt-2 text-xs text-gray-400">
                                                        <Spinner classNames={{ label: "text-foreground mt-4" }} size='sm' variant="gradient" />
                                                        <span>{message.statusStr}</span>
                                                    </div>
                                                )
                                            }
                                            {
                                                message.status === 'done' && (
                                                    <div className="flex items-center gap-1.5 mt-2 text-xs text-gray-400">
                                                        <DataArrival theme="outline" size="16" fill="#19be6b" />
                                                        <span>{message.statusStr}</span>
                                                    </div>
                                                )
                                            }
                                    ...
                                    indicator={
                                        <ExpandDown theme="outline" size="24" fill="#333" />
                                    }
                                // title={

                                //     <div className="flex items-center gap-1.5 mt-2 text-xs text-gray-400">
                                //         <ErrorIcon theme="outline" size="24" fill="#F31260" color='red' />
                                //         生成失败
                                //     </div>
                                // }
                                >
...
export default function Main() {
    ...
    const sendMsg = async function () {
        ...
        const userMsg: ChatMessage = {
            role: 'user',
            content: [{ type: 'text', id: null, status: 'done', createdAt: timestamp, text: inputText }],
            status: 'done',
            createdAt: timestamp
        };
        setInputText('')
        ...
        //发送请求
        await apiService.sseEventSource(`/SQLChat`, userMsg,
            //请求成功后
            () => {
    ...
    //分发处理sse消息
    const dispatcherOnMessage = function (msg: EventSourceMessage) {
        switch (msg.event) {
            case 'session':
                window.history.replaceState(null, '', `/chat/${msg.data}`);
                break
            case 'message':
                handlerMessage(msg.data)
                break
            case 'block':
                handlerBlock(msg.data)
                break
            case 'delta':
                handlerDelta(msg.data)
                break
            case 'message-status':
                handlerMessageStatus(msg.data)
                break
            case 'block-status':
                handlerBlockStatus(msg.data);
                break
            default:
                break;
        }
    ...
    //修改消息状态
    const handlerMessageStatus = function (data: string) {
    ...
    //修改块状态
    const handlerBlockStatus = function (data: string) {
        const json = JSON.parse(data);
    ...
    //新增消息
    const handlerMessage = function (data: string) {
        const botMsg = JSON.parse(data) as ChatMessage;
        setMockMessages((prev) => {
    ...
    //新增块
    const handlerBlock = function (data: string) {
        //找到最后一个消息，新增一个块
        setMockMessages((prev) => {
            if (prev.length === 0) return prev;
            ...
            const newContent = [...lastMessage.content, newBlock];
            ...
            const newMessages = [...prev];
            newMessages[lastIndex] = updatedLastMessage;
    ...
    //处理流式文本
    const handlerDelta = function (deltaText: string) {
        setMockMessages((prev) => {
            if (prev.length === 0) return prev;
            ...
            // 3. 校验状态：只处理状态为 streaming 的消息
            if (lastMsg.status !== 'streaming') {
                return prev;
            }

            // 4. 复制 content 数组以保持不可变性
            const newContent = [...lastMsg.content];
            const lastBlockIndex = newContent.length - 1;
            const lastBlock = lastBlockIndex >= 0 ? newContent[lastBlockIndex] : null;

            // 5. 核心逻辑：判断最后一个 block 类型
            if (lastBlock && lastBlock.type === 'text' && lastBlock.status == 'streaming') {
                const textBlock = lastBlock as ContentBlockText
                newContent[lastBlockIndex] = {
                    ...lastBlock,
                    text: (textBlock.text || "") + deltaText
                };
            } else {
                newContent.push({
    ...
    return (
        <>
            ...
            {/* 对话区域 */}
            <div className="w-full container mx-auto  sm:max-w-3xl ">
